...
LOCAL_CONFIG="$PROJECT_ROOT/config/config-service.toml"
...
SCRIPT_DIR=$(dirname "$(readlink -f "$0")")
PROJECT_ROOT=$(dirname "$SCRIPT_DIR")
LOCAL_ACTION_CONFIG="$PROJECT_ROOT/config/config-action.toml"
...
sudo chown root:root "$CONFIG_DIR/config-service.toml"
sudo chmod 644 "$CONFIG_DIR/config-service.toml"

# Set permissions for action config if it exists
if [[ -f "$CONFIG_DIR/config-action.toml" ]]; then
    sudo chown root:root "$CONFIG_DIR/config-action.toml"
    sudo chmod 644 "$CONFIG_DIR/config-action.toml"
fi

echo "Installation complete!"
echo "Binary: $INSTALL_DIR/$SERVICE_NAME"
echo "Config: $CONFIG_DIR/config-service.toml"
echo "Logs: $LOG_DIR/"
echo ""
