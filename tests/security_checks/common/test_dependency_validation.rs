...
mod tests {
    ...
    fn test_all_dependencies_declared() {
        ...
        assert!(cargo_content.contains("users ="), "Missing users dependency");
        assert!(cargo_content.contains("libc ="), "Missing libc dependency");
        assert!(cargo_content.contains("thiserror ="), "Missing thiserror dependency");
        assert!(cargo_content.contains("log ="), "Missing log dependency");
        assert!(cargo_content.contains("serde ="), "Missing serde dependency");
    }
}
