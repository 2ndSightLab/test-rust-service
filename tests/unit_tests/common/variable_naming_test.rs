...
mod tests {
    ...
    fn test_screaming_snake_case_variables() {
        ...
        let non_screaming_regex = Regex::new(r"let\s+([a-z][a-zA-Z0-9_]*|[A-Z][a-z][a-zA-Z0-9_]*)\s*[=:]").unwrap();
            ...
            if let Ok(content) = fs::read_to_string(path) {
                for (line_num, line) in content.lines().enumerate() {
                    if let Some(captures) = regex.captures(line) {
                        ...
                        if !var_name.chars().all(|c| c.is_uppercase() || c.is_numeric() || c == '_') {
                            violations.push(format!("{}:{}: Variable '{}' not in SCREAMING_SNAKE_CASE", 
                                                  path.display(), line_num + 1, var_name));
                        }
                    }
                }
            }
        ...
        if !violations.is_empty() {
            panic!("Found variables not in SCREAMING_SNAKE_CASE:\n{}", violations.join("\n"));
        }
    }
}
